...
export default function ProjectHeader({
    ...
    return (
        <div className="grid grid-cols-3 p-1.5 gap-5 place-items-center text-left">
            ...
            <h3 className={`justify-self-start text-md md:text-xl pl-0 font-medium transition-[opacity,scale,padding] duration-700 ease-in-out ${isExpanded ? "scale-150 pl-5" : ""}`}>
                {name}
            </h3>

            {/* 🔹 Tech Icons (only collapsed) */}
            <div className={`justify-self-start grid grid-cols-2 sm:grid-cols-4 place-items-center gap-3 text-xl transition-opacity duration-500 ${isExpanded ? "opacity-0" : "opacity-100"}`}>
                {icons.map((icon, idx) => (
