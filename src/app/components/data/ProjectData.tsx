...
// 1. Localized fields (content that changes based on language)
...
// 2. Source Data Structure (Nested storage)
// This is how we store data in the array below.
interface ProjectDataSource {
  id: number;
  key: string;
  name: string; // Shared name across languages
  date: string;
  icons: JSX.Element[];
  images: string[];
  href?: string;
...
// 3. Result Type for Components (Flattened)
// We remove the nested 'content' object and merge its fields (review, etc.) to the top level.
// This ensures components like ProjectItem access data easily (e.g., project.review).
export interface Project extends Omit<ProjectDataSource, 'content'>, LocalizedContent {}

// ==========================================
// Projects Data (Single Source of Truth)
// ==========================================
const projectsData: ProjectDataSource[] = [
  {
    id: 1,
    key: "pck",
    name: "PCK",
...
export const getProjects = (lang: Language): Project[] => {
  return projectsData.map((project) => {
    // 1. Destructure: separate 'content' and the rest ('baseData' + 'name')
    const { content, ...baseData } = project;
    
    // 2. Select content for the specific language
    const localizedContent = content[lang];

    // 3. Merge into a single flat object
    return {
      ...baseData,
      ...localizedContent,
    };
